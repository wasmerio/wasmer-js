...
async function createIframeAndCommunicate() {
  ...
  await init({ log: "debug", module: WasmModule });

  const network = new HttpListenerNetworking();
  const runtime = new Runtime({ networking: network.clone() });
  network.setOnNewListener(runtime, (addr?: string) => {
    console.log("NEW LISTENER! " + addr);
  });
  const pkg = await Wasmer.fromRegistry("wasmer/hello", runtime);
  console.log("pkg");
  ...
  instance.stdout.pipeTo(
    new WritableStream({
      write: chunk => console.log(utf8_decoder.decode(chunk)),
    }),
  );
  ...
  setTimeout(async () => {
    ...
    console.log("Sending request network", network);
    let response = await network.handleRequest(
      request,
      "[::]:80",
      "127.0.0.4:50000",
    );
  ...
  iframe.title =
    "Embedded browser. It shows the responses from the environment";
