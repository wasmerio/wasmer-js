...
use wasmer_wasix::virtual_fs::{ArcFile, FsError, VirtualFile};

fn assert_send_sync<T: Send + Sync>() {}

const _: () = {
    assert_send_sync::<PipeTx<Bytes>>();
    assert_send_sync::<PipeRx<Bytes>>();
    assert_send_sync::<Pipe<Bytes, Bytes>>();
};

#[derive(Debug, Clone)]
pub struct Pipe<Tx, Rx> {
    send: PipeTx<Tx>,
    recv: PipeRx<Rx>,
}
...
pub struct PipeTx<T> {
    ...
    tx: Arc<Mutex<mpsc::UnboundedSender<T>>>,
...
struct PipeReceiver<R> {
    chan: mpsc::UnboundedReceiver<R>,
    buffer: Option<Bytes>,
}

impl<Tx, Rx> Pipe<Tx, Rx> {
    fn new() -> Self {
    ...
    pub fn channel() -> (Self, Self) {
        let (tx1, rx1) = Self::new().split();
        let (tx2, rx2) = Self::new().split();

        let end1 = Self::combine(tx1, rx2);
        let end2 = Self::combine(tx2, rx1);
        (end1, end2)
    }
    pub fn split(self) -> (PipeTx<Tx>, PipeRx<Rx>) {
        (self.send, self.recv)
    }
    pub fn combine(tx: PipeTx<Tx>, rx: PipeRx<Rx>) -> Self {
