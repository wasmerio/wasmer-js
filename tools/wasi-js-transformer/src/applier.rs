...
pub fn apply_transformations_to_wasm_binary_vec(
    ...
    let types_section = wasm_sections
        ...
        .unwrap();
    let lowered_signature_bytes = Vec::new();
    for lowered_signature in lowered_signatures.iter() {
        lowered_signature_bytes.push(lowered_signature.bytes);
    }
    ...
    for i in 0..imported_i64_functions.len() {
        let imported_i64_function = imported_i64_functions.get(i);
        ...
        let import_function_signature_position = field_length_start_position
            ...
            + import_field_name_length;

        // Get the signature byte length (to remove later)
        let (_, import_function_signature_byte_length) = read_bytes_as_varunit32_and_byte_length(
            wasm_binary_vec
    ...
    let functions_section = wasm_sections
        ...
        .unwrap();
    let trampoline_signatures = Vec::new();
    for trampoline_function in trampoline_functions.iter() {
        let trampoline_signature_bytes =
            get_u32_as_bytes_for_varunit32(trampoline_function.signature_index);
        trampoline_signatures.push(trampoline_signature_bytes);
    }
    position_offset += add_entries_to_section(
        wasm_binary_vec,
        &mut position_offset,
    ...
    for i in 0..imported_i64_functions.len() {
            ...
            .filter(|&x| x.function_index == imported_i64_function.function_index)
        {
            ...
            remove_number_of_bytes_at_position(&mut wasm_binary_vec, start, call_index_byte_length);
    ...
    let code_section = wasm_sections
        ...
        .unwrap();
    let trampoline_function_bytes = Vec::new();
    for trampoline_function in trampoline_functions.iter() {
        trampoline_function_bytes.push(trampoline_function.bytes);
    }
    position_offset += add_entries_to_section(
        wasm_binary_vec,
        &mut position_offset,
...
fn add_entries_to_section(
    ...
    for entry in entries.iter() {
        for i in 0..entry.len() {
            wasm_binary_vec.insert(types_section.end_position + i, entry.get(i).unwrap());
            bytes_added += 1;
        }
    }
    ...
    let (section_length, section_length_byte_length) = read_bytes_as_varunit32_and_byte_length(
        wasm_binary_vec
            .get(section_length_position..(section_length_position + 4))
            .unwrap(),
    );
    let new_section_length = original_code_section_length + bytes_added_to_code_section as u32;
    let new_section_length_bytes = get_u32_as_bytes_for_varunit32(new_section_length);
    ...
    position_offset += size_length_difference;
    ...
    let (number_of_entries, number_of_entries_byte_length) =
        read_bytes_as_varunit32_and_byte_length(
            wasm_binary_vec
    ...
    let number_of_entries_bytes = get_u32_as_bytes_for_varunit32(number_of_entries);
    remove_number_of_bytes_at_position(
        &mut wasm_binary_vec,
    ...
    position_offset += size_count_difference;
